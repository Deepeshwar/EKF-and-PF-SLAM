...
class Particle:
    ...
    def h_expected_measurement_for_landmark(self, landmark_number,
        ...
        Exp_meas = self.h(self.pose, self.landmark_positions[landmark_number], scanner_displacement)
    ...
    def wl_likelihood_of_correspondence(self, measurement,
        ...
        A = np.dot((np.transpose(delta_z)), np.dot(np.linalg.inv(Ql), delta_z))

        l = (1/(2*pi*sqrt(p))) * exp(-0.5 * A)
    ...
    def initialize_new_landmark(self, measurement_in_scanner_system,
        ...
        new_covariance = np.dot(H_inv, np.dot(Qt_measurement_covariance, np.transpose(H_inv)))
        
        self.landmark_positions.append(m)
        self.landmark_covariances.append(new_covariance)  
    ...
    def update_landmark(self, landmark_number, measurement,
        ...
        K = np.dot(sigma_old, np.dot(np.transpose(H), Ql_inv))
        
        Exp_meas = self.h_expected_measurement_for_landmark(landmark_number, scanner_displacement)
        Delta_z = measurement - Exp_meas
        KH = np.dot(K, H)
        n = KH.shape[0];
        self.landmark_positions[landmark_number] += np.dot(K, Delta_z)
        self.landmark_covariances[landmark_number] = np.dot((np.eye(n) - KH), sigma_old)
    ...
    def update_particle(self, measurement, measurement_in_scanner_system,
        ...
        # Else update the particle's EKF for the corresponding particle.
            ...
            w, index = Max, ArgMax  # Replace this by a search for max, argmax.
    ...
    def decrement_visible_landmark_counters(self, scanner_displacement):
        ...
        for i in range(number_of_landmarks):
            print("Hello")         
            bearing_angle = self.h_expected_measurement_for_landmark(i, scanner_displacement)
            print("heuu")
            Min, Max = LegoLogfile.min_max_bearing()
            if bearing_angle > Min and bearing_angle < Max:
    ...
    def remove_spurious_landmarks(self):
        ...
        # --->>> Insert your new code here.
        number_of_landmark = self.number_of_landmarks()
        
        new_pos_list = []
        new_cov_list = []
        
        for i in range(number_of_landmark):
            m = self.landmark_counters[i]
            if m > 0:
                new_pos_list.append(self.landmark_positions[i])
                new_cov_list.append(self.landmark_covariances[i])
            else:
                new_pos_list.append(0)
                new_cov_list.append(0)
                
        # Remove any landmark for which the landmark_counters[]
        # Note: deleting elements of a list while iterating over the list
        # will not work properly. One relatively simple and ele is (strictly)
        # smaller than zero.gant solution is
