...
class ExtendedKalmanFilterSLAM:
    ...
    def predict(self, control):
        ...
        G[0:3, 0:3] = G3
        G[0:3, 3:3+2*n] = zeros((3, (2*n)))
        G[3:3+2*n, 0:3] = zeros(((2*n), 3))
        G[3:3+2*n, 3:3+2*n] = eye(2*n)
        ...
        R[0:3, 0:3] = R3
        R[0:3, 3:3+2*n] = zeros((3, (2*n)))
        R[3:3+2*n, 0:3] = zeros(((2*n), 3))
        R[3:3+2*n, 3:3+2*n] = zeros(2*n)
    ...
    def add_landmark_to_state(self, initial_coords):
        ...
        x, y = initial_coords
        new_landmark = array([x, y])
               
        dim_x, dim_y = self.covariance.shape
        K = zeros((dim_x+2, dim_y+2))
        K[0:dim_x, 0:dim_y] = self.covariance
        K[dim_x:,dim_y:] = diag([10**10,10**10])
        ...
        M = hstack((self.state[:,], new_landmark[:,]))
        self.state = M
...
if __name__ == '__main__':
    ...
    initial_state = array([500.0, 200.0, 45.0 / 180.0 * pi])
    ...
    for i in xrange(len(logfile.motor_ticks)):
        ...
        for obs in observations:    
            ...
            if cylinder_index == -1:        #if no closest cylinder found the it returns -1
